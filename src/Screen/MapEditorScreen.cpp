...
void MapEditorScreen::InitializeTilesets() {
    ...
    // TILES (0-103): Basic terrain tiles
    for (int i = 0; i <= 103; i++) {
        TilesetTile tile;
        tile.tileId = i;
    ...
    std::vector<int> blockIds = {104, 105, 106, 107, 111, 114, 115, 116, 117, 119, 120};
    ...
    for (int itemId : itemIds) {
        ...
        std::string textureName = "ITEM_" + std::to_string(itemId);
        try {
            item.texture = ResrcManager::GetInstance().getTexture(textureName);
        } catch (...) {
            item.texture = {0};
        }
...
void MapEditorScreen::Update() {
   ...
   if (IsKeyPressed(KEY_ONE)) currentMode = EditorMode::TILE_MODE;
   if (IsKeyPressed(KEY_TWO)) currentMode = EditorMode::BLOCK_MODE;
   if (IsKeyPressed(KEY_THREE)) currentMode = EditorMode::ENEMY_MODE;
   if (IsKeyPressed(KEY_FOUR)) currentMode = EditorMode::ITEM_MODE;
   UpdateCamera();
   HandleUIEvents();
       if (editorActive) {
        HandleMapInput();
        ...
        for (auto& tile : currentMap->getInteractiveTiles()) {
            if (tile) {
                ...
                if (x >= 0 && x < 150 && y >= 0 && y < 30) {
                    layerData[y * 150 + x] = tile->GetKey() + 1; // +1 because Tiled uses 1-based indexing
                }
            }
        }
...
int MapEditorScreen::GetBlockTileId(Block* block) {
    // You need to implement reverse mapping from block type to tile ID
    // This depends on your block class hierarchy
    if (dynamic_cast<QuestionBlock*>(block) && block->GetGiftType() == GIFT_NONE) return 115;
    if (dynamic_cast<QuestionBlock*>(block) && block->GetGiftType() == GIFT_COIN) return 116;
    if (dynamic_cast<QuestionBlock*>(block) && block->GetGiftType() == GIFT_FIRE_FLOWER) return 117;
    if (dynamic_cast<WoodBlock*>(block)) return 120;
    if (dynamic_cast<GlassBlock*>(block)) return 111;
    if (dynamic_cast<CloudBlock*>(block)) return 104;
    if (dynamic_cast<EyesClosedBlock*>(block)) return 106; // Closed eye
    if (dynamic_cast<EyesOpenedBlock*>(block)) return 107; // Open eye
    if (dynamic_cast<WoodBlock*>(block)) return 120;
...
int MapEditorScreen::GetEnemyTileId(Enemy* enemy) {
    ...
    if (dynamic_cast<Goomba*>(enemy)) return 126;
    if (dynamic_cast<GreenKoopa*>(enemy)) return 127;
    if (dynamic_cast<RedKoopa*>(enemy)) return 133;
    if (dynamic_cast<Rex*>(enemy)) return 134;
    if (dynamic_cast<Bob_omb*>(enemy)) return 122;
    if (dynamic_cast<BuzzyBeetle*>(enemy)) return 124;
    if (dynamic_cast<FlyingGoomba*>(enemy)) return 125;
...
int MapEditorScreen::GetItemTileId(Item* item) {
    ...
    if (dynamic_cast<Coin*>(item)) return 139;
    if (dynamic_cast<Mushroom*>(item)) return 141;
    if (dynamic_cast<FireFlower*>(item)) return 140;
    if (dynamic_cast<Star*>(item)) return 142;
    if (dynamic_cast<OneUpMushroom*>(item)) return 137;
    if (dynamic_cast<ThreeUpMoon*>(item)) return 138;
    if (dynamic_cast<YoshiCoin*>(item)) return 143;

    return 0;
}

void MapEditorScreen::HandleMapInput() {
    if (!IsMouseInMainViewport()) return;
    ...
    if (CheckCollisionPointRec(GetMousePosition(), GetTilesetPanel())) {
        ...
        if (wheelMove != 0) {
            ...
            if (tilesetScrollY < 0) tilesetScrollY = 0;
            
            // Calculate max scroll based on tileset content
            int maxRows = 0;
            switch (currentMode) {
                case EditorMode::TILE_MODE: maxRows = (availableTiles.size() + 5) / 6; break;
                case EditorMode::BLOCK_MODE: maxRows = (availableBlocks.size() + 5) / 6; break;
                case EditorMode::ENEMY_MODE: maxRows = (availableEnemies.size() + 2) / 3; break;
                case EditorMode::ITEM_MODE: maxRows = (availableItems.size() + 5) / 6; break;
            }
            
            float maxScroll = std::max(0.0f, maxRows * 75.0f - GetTilesetPanel().height + 100);
            if (tilesetScrollY > maxScroll) tilesetScrollY = maxScroll;
        }
    }
}
...
void MapEditorScreen::PlaceObject() {
    ...
    switch (currentMode) {
        case EditorMode::TILE_MODE: {
            // Add tile to map
            if (selectedTileId >= 0 && selectedTileId <= 103) {
        ...
        case EditorMode::BLOCK_MODE: {
            // Add block to map
            if (selectedTileId >= 104 && selectedTileId <= 120) {
                RemoveObjectAt(tileX, tileY, currentMode);
    ...
    switch (mode) {
        ...
        case EditorMode::ENEMY_MODE: {
            ...
            for (auto it = enemies.begin(); it != enemies.end(); ++it) {
                if (*it && fabs((*it)->GetPos().x - worldPos.x) < TOLERANCE && 
                    fabs((*it)->GetPos().y - worldPos.y + 20) < TOLERANCE) {
                    delete *it;
                    enemies.erase(it);
...
Block* MapEditorScreen::CreateBlockByType(int tileId, Vector2 position) {
    switch (tileId) {
        case 104: return new CloudBlock(position, {32, 32}, WHITE);
        case 106: return new EyesOpenedBlock(position, {32, 32}, WHITE);
        case 107: return new EyesClosedBlock(position, {32, 32}, WHITE);
        case 111: return new GlassBlock(position, {32, 32}, WHITE);
        case 115: return new QuestionBlock(position, {32, 32}, WHITE, GIFT_NONE);
        case 116: return new QuestionBlock(position, {32, 32}, WHITE, GIFT_COIN);
        case 117: return new QuestionBlock(position, {32, 32}, WHITE, GIFT_FIRE_FLOWER);
        case 120: return new WoodBlock(position, {32, 32}, WHITE);
...
Enemy* MapEditorScreen::CreateEnemyByType(int tileId, Vector2 position) {
    switch (tileId) {
        case 122: return new Bob_omb(position);
        case 124: return new BuzzyBeetle(position);
        case 125: return new FlyingGoomba(position);
        case 126: return new Goomba(position);
        case 127: return new GreenKoopa(position);
        case 133: return new RedKoopa(position);
        case 134: return new Rex(position);
        default: return nullptr;
    }
}
...
std::shared_ptr<Item> MapEditorScreen::CreateItemByType(int tileId, Vector2 position) {
    switch (tileId) {
        case 137: return std::make_shared<OneUpMushroom>(position);
        case 138: return std::make_shared<ThreeUpMoon>(position);
        case 139: return std::make_shared<Coin>(position);
        case 140: return std::make_shared<FireFlower>(position);
        case 141: return std::make_shared<Mushroom>(position);
        case 142: return std::make_shared<Star>(position);
        case 143: return std::make_shared<YoshiCoin>(position);
        default: return nullptr;
    }
}
...
void MapEditorScreen::DrawCursorPreview() {
    ...
    switch (currentMode) {
        case EditorMode::TILE_MODE: previewColor = BLUE; break;
        case EditorMode::BLOCK_MODE: previewColor = ORANGE; break;
        case EditorMode::ENEMY_MODE: previewColor = RED; break;
        case EditorMode::ITEM_MODE: previewColor = GOLD; break;
    }
...
void MapEditorScreen::DrawTilesetContent(Rectangle area) {
   ...
   switch (currentMode) {
      case EditorMode::TILE_MODE: currentTileset = &availableTiles; break;
      case EditorMode::BLOCK_MODE: currentTileset = &availableBlocks; break;
      case EditorMode::ENEMY_MODE: currentTileset = &availableEnemies; break;
      case EditorMode::ITEM_MODE: currentTileset = &availableItems; break;
   }
...
void MapEditorScreen::UpdateCamera() {
   ...
   if (IsKeyDown(KEY_W)) camera.target.y -= CAMERA_SPEED * GetFrameTime();
   if (IsKeyDown(KEY_S)) camera.target.y += CAMERA_SPEED * GetFrameTime();
   if (IsKeyDown(KEY_A)) camera.target.x -= CAMERA_SPEED * GetFrameTime();
   if (IsKeyDown(KEY_D)) camera.target.x += CAMERA_SPEED * GetFrameTime();
...
void MapEditorScreen::HandleUIEvents() {
   if (showLoadDialog) {
      ...
      if (wheelMove != 0) {
         ...
         if (mapListScrollY < 0) mapListScrollY = 0;
         if (mapListScrollY > maxScroll) mapListScrollY = maxScroll;
      }
