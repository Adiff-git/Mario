...
void Character::SetSprite(Texture2D sprite) {
    this->sprite = &sprite; // Assuming sprite is a Texture2D reference
}
void Character::SetState(ObjectState state) {
    this->state = state;
}
...
void Character::SetMarioState(ObjectState state)
{
    ...
    if (state == SMALL) {
        ...
        SetSize(Vector2{32, 56});
        sprite = &ResrcManager::GetInstance().getTexture("BIGMARIO_0_RIGHT");
    } else if (state == FIRE) {
        SetSize(Vector2{32, 56});
        sprite = &ResrcManager::GetInstance().getTexture("FIREMARIO_0_RIGHT");
    }
}

bool Character::GetIsDucking() const
{
    return isDucking;
}
...
void Character::AddObserver(Observer* ob) {
    observers.push_back(ob);
}

void Character::RemoveObserver(Observer* ob) {
    observers.erase(std::remove(observers.begin(), observers.end(), ob), observers.end());
}

void Character::NotifyCoinChange() {
