...
// why we use a pointer here? Because we want to ensure that the instance is created only once and can be accessed globally.
    GameClock() = default;
    ~GameClock(); // Make destructor public to handle cleanup
public:
    static  GameClock &GetInstance(); // Get the singleton instance
    double updateTimeAcum; // Accumulated time for updat// what is this used for? This is used to accumulate the time between frames so that we can update the game logic at a fixed time step.
